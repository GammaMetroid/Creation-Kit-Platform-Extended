	...
	namespace SkyrimSE
	{
		namespace EditorAPI
		{
			...
			public:
				...
				[[nodiscard]] virtual inline bool IsNotEqual(const BSExtraData* rhs) const noexcept(true) { return false; }

				[[nodiscard]] bool operator==(const BSExtraData& rhs) const noexcept(true);
				[[nodiscard]] bool operator!=(const BSExtraData& rhs) const noexcept(true);	
			...
			class ExtraStartingPosition : public BSExtraData
			{
				...
				char unk28[8];				// location form ?
			public:
				static constexpr auto TYPE{ kStartingPosition };

				ExtraStartingPosition() : BSExtraData() {}
				ExtraStartingPosition(const NiAPI::NiPoint3& Position, const NiAPI::NiPoint3& Rotate) :
					BSExtraData(), _position(Position), _rotate(Rotate) {}
				...
				[[nodiscard]] inline NiAPI::NiPoint3& GetRotate() noexcept(true) { return _rotate; }
			};
			...
			public:
				...
				ExtraEditorRef3DData() : BSExtraData() {}
				ExtraEditorRef3DData(const NiAPI::NiPointer<NiAPI::NiAVObject>& Model3D) : BSExtraData(), _3D(Model3D) {}
			...
			public:
				...
				ExtraEditorRefMoveData() : BSExtraData() {}
				ExtraEditorRefMoveData(NiAPI::NiPoint3 Angle, NiAPI::NiPoint3 OldPosition, NiAPI::NiPoint3 Position) :
					BSExtraData(), _Angle(Angle), _OldPosition(OldPosition), _Position(Position) {}
			...
			public:
				...
				ExtraPrimitive(BGSPrimitive* Primitive) : BSExtraData(), _Primitive(Primitive) {}
			...
			{
				BSOcclusionPlane* _OcclusionShape{ nullptr };
			public:
				static constexpr auto TYPE{ kOcclusionShape };

				ExtraOcclusionShape(BSOcclusionPlane* OcclusionShape) : BSExtraData(), _OcclusionShape(OcclusionShape) {}
				virtual ~ExtraOcclusionShape() = default;
